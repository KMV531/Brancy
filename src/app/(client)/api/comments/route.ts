...
// /pages/api/comments.ts (or /app/api/comments/route.ts if using app router)
export async function GET(req: Request) {
  ...
  const query = `*[_type == "blog" && _id == $blogId][0].comments[isApproved == true]`;
  const comments = await client.fetch(query, { blogId });
